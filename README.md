...
| Random | `--mode random` | 从 `time`、`weather`、`mood`、`random` 素材池随机选择，避免连续重复 |
...
```text
backgrounds/
├── current.jpg                       # Ghostty 当前实际读取的运行时图片
├── time/
│   ├── morning/
│   ├── afternoon/
│   └── evening/
...
新结构按“模式”分层，避免 `focus`、`clear`、`morning` 等分类全部挤在根目录。CLI 仍会兼容旧版平铺目录，例如 `backgrounds/focus/`，但新素材建议放入对应命名空间。

替换图片或使用外部图片库：

```bash
cp ~/Pictures/my-focus-image.jpg ~/.config/ghostty/backgrounds/mood/focus/
...
支持 `.jpg`、`.jpeg` 和 `.png`。`GHOSTTY_BACKGROUND_DIR` 指向外部图片库时也建议使用同样的 `time/`、`weather/`、`mood/`、`random/` 结构。下载的图片请在 [`SOURCES.md`](chezmoi/dot_config/ghostty/backgrounds/SOURCES.md) 中记录来源和授权。
...
│   │       └── backgrounds/          # current.jpg 与 time/weather/mood/random 素材
│   ├── dot_local/bin/
